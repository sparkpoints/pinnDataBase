...
class slicesView():
    ...
    def __init__(self, data=[], figure=-1, slices=[],
        ...
        self._ax = []
        self._ax.append(self._fig.add_axes(
            [lftBrdr, btmBrdr + btmFigH + midBrdrH, topFigW, topFigH]))
        self._ax.append(self._fig.add_axes(
            [lftBrdr, btmBrdr, btmFigW, btmFigH]))
        self._ax.append(self._fig.add_axes(
    ...
    def refreshIm(self, ax):
        ...
        if self._axOrien[ax] == 0:
            ...
            self._ax[ax].set_aspect(
                self._im1_voxSize[1] / self._im1_voxSize[2])
        elif self._axOrien[ax] == 1:
            self._im1_dispSlices[ax] = self._im1_data[:,
                                                      self._im1_slice[ax], :]
            self._ax[ax].set_aspect(
                self._im1_voxSize[0] / self._im1_voxSize[2])
        else:
            self._im1_dispSlices[ax] = self._im1_data[:,
                                                      :, self._im1_slice[ax]]
            self._ax[ax].set_aspect(
                self._im1_voxSize[0] / self._im1_voxSize[1])
    ...
    def initializeImageSlices(self):
        ...
        for ax in range(3):
            if self._axOrien[ax] == 0:
                self._im1_dispSlices.append(
                    self._im1_data[self._im1_slice[ax], :, :])
            elif self._axOrien[ax] == 1:
                self._im1_dispSlices.append(
                    self._im1_data[:, self._im1_slice[ax], :])
            else:
    ...
    def processArguments(self, data, slices, startP, voxSize, cmap, im1_data, im1_startP,
        ...
        self._im1_axes = self.setImageAxes(
            self._im1_data.shape, self._im1_voxSize, self._im1_startP)
        self._im1_extent = [[self._im1_axes[2][0], self._im1_axes[2][-1], self._im1_axes[1][0], self._im1_axes[1][-1]],
                            [self._im1_axes[2][0], self._im1_axes[2][-1],
                             self._im1_axes[0][0], self._im1_axes[0][-1]],
                            [self._im1_axes[1][0], self._im1_axes[1][-1], self._im1_axes[0][0], self._im1_axes[0][-1]]]

        if self._plotIm2:
            self._im2_axes = self.setImageAxes(
                self._im2_data.shape, self._im2_voxSize, self._im2_startP)
            self._im2_extent = [[self._im2_axes[2][0], self._im2_axes[2][-1], self._im2_axes[1][0], self._im2_axes[1][-1]],
                                [self._im2_axes[2][0], self._im2_axes[2][-1],
                                 self._im2_axes[0][0], self._im2_axes[0][-1]],
    ...
    def setSecondarySlice(self, im_axes, imData_shape, ax):
        ...
        if self._axOrien[ax] == 0:
            im_slice = np.interp(
                self._im1_axes[0][self._im1_slice[ax]], im_axes[0], np.arange(imData_shape[0]))
        elif self._axOrien[ax] == 1:
            im_slice = np.interp(
                self._im1_axes[1][self._im1_slice[ax]], im_axes[1], np.arange(imData_shape[1]))
        else:
            im_slice = np.interp(
                self._im1_axes[2][self._im1_slice[ax]], im_axes[2], np.arange(imData_shape[2]))
    ...
    def interpSecondaryLinear(self, imData, imSlice, voxSize, ax):
        ...
        if self._axOrien[ax] == 0:
            ...
            wt0 = 1.0 - wt1
            dispSlice = imData[int(slice_flr), :, :] * wt0 + \
                imData[int(slice_flr + 1), :, :] * wt1
            ...
            wt0 = 1.0 - wt1
            dispSlice = imData[:, :, int(slice_flr)] * \
                wt0 + imData[:, :, int(slice_flr + 1)] * wt1
    ...
    def connect(self):
        ...
        self._evkp = self._fig.canvas.mpl_connect(
    ...
    def axToAxial(self):
        ...
        if self._axOrien[self._curAx] != 0:
            ...
            self._ax[self._curAx].set_aspect(
                self._im1_voxSize[1] / self._im1_voxSize[2])
            self.refreshIm(self._curAx)

...
def show_bland_altman_plot(title, data1, data2, *args, **kwargs):
    ...
    # Standard deviation of the difference (ddof = 1 for corrected (unbiased) estimator)
    sd = np.std(diff, axis=0, ddof=1)
    semd = np.sqrt(np.square(sd) / n)        # Standard error of md
    # Confidence Interval offset for Mean of differences
    ciMd = t * semd
    rc = 1.96 * sd                         # repeatability coefficient
    # limit of agreement (LOA) upper bound
    loaUp = md + rc
    # limit of agreement (LOA) lower bound
    loaLow = md - rc

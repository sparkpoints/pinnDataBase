...
def Read_scan(path): # Read scans under the specified path 
    scan = [pydicom.dcmread(s, force=True) for s in glob.glob(os.path.join(path,'CT*.dcm'))]
    try:
...
def Read_RTSTRUCT(path): # Read RTSTRUCT under the specified path
    try:
        rt = [pydicom.dcmread(s, force=True) for s in glob.glob(os.path.join(path,'RS*.dcm'))]
    except:
...
if __name__ == "__main__":
    parseDCMRT('/Users/yang/Downloads/483150')
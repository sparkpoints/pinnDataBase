...
def main():
    ...
    rtplanfile = ''
    rtdosefile = ''
    for file in glob.glob(os.path.join(path,'*.dcm')):
        if 'RS' in os.path.basename(file).upper():
            rtssfile = os.path.join(path, file)
        if 'RP' in os.path.basename(file).upper():
            rtplanfile = os.path.join(path, file)
        if 'RD' in os.path.basename(file).upper():
            rtdosefile = os.path.join(path, file)
    ...
    result_1 = extractor.execute(new_img, label_1)
    ...
    for key, structure in RTstructures.items():
        ...
        calcdvhs[key] = get_dvh(rtssfile, rtdosefile, key)
